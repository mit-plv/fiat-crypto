...
func Divstep(out1 *uint32, out2 *[9]uint32, out3 *[9]uint32, out4 *[8]uint32, out5 *[8]uint32, arg1 uint32, arg2 *[9]uint32, arg3 *[9]uint32, arg4 *[8]uint32, arg5 *[8]uint32) {
	...
	var x185 uint32
	var x186 uint32
	x185, x186 = bits.Sub32(x169, 0xd0364141, uint32(0x0))
	var x187 uint32
	var x188 uint32
	x187, x188 = bits.Sub32(x171, 0xbfd25e8c, uint32(uint1(x186)))
	var x189 uint32
	var x190 uint32
	x189, x190 = bits.Sub32(x173, 0xaf48a03b, uint32(uint1(x188)))
	var x191 uint32
	var x192 uint32
	x191, x192 = bits.Sub32(x175, 0xbaaedce6, uint32(uint1(x190)))
	var x193 uint32
	var x194 uint32
	x193, x194 = bits.Sub32(x177, 0xfffffffe, uint32(uint1(x192)))
	var x195 uint32
	var x196 uint32
