/* Autogenerated: src/ExtractionOCaml/bedrock2_unsaturated_solinas --lang=bedrock2 --no-wide-int --widen-carry --widen-bytes --split-multiret poly1305 3 '2^130 - 5' 64 carry_mul carry_square carry add sub opp from_bytes */
/* curve description: poly1305 */
/* requested operations: carry_mul, carry_square, carry, add, sub, opp, from_bytes */
/* n = 3 (from "3") */
/* s-c = 2^130 - [(1, 5)] (from "2^130 - 5") */
/* machine_wordsize = 64 (from "64") */

/* Computed values: */
/* carry_chain = [0, 1, 2, 0, 1] */

#include <stdint.h>


/*
 * Input Bounds:
 *   in0: [[0x0 ~> 0x34cccccccccb], [0x0 ~> 0x1a6666666664], [0x0 ~> 0x1a6666666664]]
 *   in1: [[0x0 ~> 0x34cccccccccb], [0x0 ~> 0x1a6666666664], [0x0 ~> 0x1a6666666664]]
 * Output Bounds:
 *   out0: [[0x0 ~> 0x119999999999], [0x0 ~> 0x8cccccccccc], [0x0 ~> 0x8cccccccccc]]
 */
void fiat_poly1305_carry_mul(uintptr_t in0, uintptr_t in1, uintptr_t out0) {
  uintptr_t x2, x1, x5, x4, x0, x3, x8, x10, x25, x11, x26, x9, x24, x22, x29, x23, x30, x27, x31, x28, x12, x14, x35, x15, x36, x13, x34, x18, x39, x19, x40, x37, x6, x16, x43, x17, x44, x7, x42, x20, x47, x21, x48, x45, x46, x32, x51, x49, x52, x50, x38, x53, x56, x41, x57, x55, x58, x33, x60, x61, x62, x54, x64, x65, x59, x63, x66, x67, x68, x69, x70;
  x0 = *(uintptr_t*)((in0)+((uintptr_t)0ULL));
  x1 = *(uintptr_t*)((in0)+((uintptr_t)8ULL));
  x2 = *(uintptr_t*)((in0)+((uintptr_t)16ULL));
  /*skip*/
  x3 = *(uintptr_t*)((in1)+((uintptr_t)0ULL));
  x4 = *(uintptr_t*)((in1)+((uintptr_t)8ULL));
  x5 = *(uintptr_t*)((in1)+((uintptr_t)16ULL));
  /*skip*/
  /*skip*/
  x6 = (x2)*((x5)*((uintptr_t)5ULL));
  x7 = sizeof(intptr_t) == 4 ? ((uint64_t)(x2)*((x5)*((uintptr_t)5ULL)))>>32 : ((__uint128_t)(x2)*((x5)*((uintptr_t)5ULL)))>>64 /* TODO this has not been tested */;
  x8 = (x2)*((x4)*((uintptr_t)10ULL));
  x9 = sizeof(intptr_t) == 4 ? ((uint64_t)(x2)*((x4)*((uintptr_t)10ULL)))>>32 : ((__uint128_t)(x2)*((x4)*((uintptr_t)10ULL)))>>64 /* TODO this has not been tested */;
  x10 = (x1)*((x5)*((uintptr_t)10ULL));
  x11 = sizeof(intptr_t) == 4 ? ((uint64_t)(x1)*((x5)*((uintptr_t)10ULL)))>>32 : ((__uint128_t)(x1)*((x5)*((uintptr_t)10ULL)))>>64 /* TODO this has not been tested */;
  x12 = (x2)*(x3);
  x13 = sizeof(intptr_t) == 4 ? ((uint64_t)(x2)*(x3))>>32 : ((__uint128_t)(x2)*(x3))>>64 /* TODO this has not been tested */;
  x14 = (x1)*((x4)*((uintptr_t)2ULL));
  x15 = sizeof(intptr_t) == 4 ? ((uint64_t)(x1)*((x4)*((uintptr_t)2ULL)))>>32 : ((__uint128_t)(x1)*((x4)*((uintptr_t)2ULL)))>>64 /* TODO this has not been tested */;
  x16 = (x1)*(x3);
  x17 = sizeof(intptr_t) == 4 ? ((uint64_t)(x1)*(x3))>>32 : ((__uint128_t)(x1)*(x3))>>64 /* TODO this has not been tested */;
  x18 = (x0)*(x5);
  x19 = sizeof(intptr_t) == 4 ? ((uint64_t)(x0)*(x5))>>32 : ((__uint128_t)(x0)*(x5))>>64 /* TODO this has not been tested */;
  x20 = (x0)*(x4);
  x21 = sizeof(intptr_t) == 4 ? ((uint64_t)(x0)*(x4))>>32 : ((__uint128_t)(x0)*(x4))>>64 /* TODO this has not been tested */;
  x22 = (x0)*(x3);
  x23 = sizeof(intptr_t) == 4 ? ((uint64_t)(x0)*(x3))>>32 : ((__uint128_t)(x0)*(x3))>>64 /* TODO this has not been tested */;
  x24 = (x10)+(x8);
  x25 = (x24)<(x10);
  x26 = (x25)+(x11);
  x27 = (x26)+(x9);
  x28 = (x22)+(x24);
  x29 = (x28)<(x22);
  x30 = (x29)+(x23);
  x31 = (x30)+(x27);
  x32 = ((x28)>>((uintptr_t)44ULL))|((x31)<<((uintptr_t)20ULL));
  x33 = (x28)&((uintptr_t)17592186044415ULL);
  x34 = (x14)+(x12);
  x35 = (x34)<(x14);
  x36 = (x35)+(x15);
  x37 = (x36)+(x13);
  x38 = (x18)+(x34);
  x39 = (x38)<(x18);
  x40 = (x39)+(x19);
  x41 = (x40)+(x37);
  x42 = (x16)+(x6);
  x43 = (x42)<(x16);
  x44 = (x43)+(x17);
  x45 = (x44)+(x7);
  x46 = (x20)+(x42);
  x47 = (x46)<(x20);
  x48 = (x47)+(x21);
  x49 = (x48)+(x45);
  x50 = (x32)+(x46);
  x51 = (x50)<(x32);
  x52 = (x51)+(x49);
  x53 = ((x50)>>((uintptr_t)43ULL))|((x52)<<((uintptr_t)21ULL));
  x54 = (x50)&((uintptr_t)8796093022207ULL);
  x55 = (x53)+(x38);
  x56 = (x55)<(x53);
  x57 = (x56)+(x41);
  x58 = ((x55)>>((uintptr_t)43ULL))|((x57)<<((uintptr_t)21ULL));
  x59 = (x55)&((uintptr_t)8796093022207ULL);
  x60 = (x58)*((uintptr_t)5ULL);
  x61 = (x33)+(x60);
  x62 = (x61)>>((uintptr_t)44ULL);
  x63 = (x61)&((uintptr_t)17592186044415ULL);
  x64 = (x62)+(x54);
  x65 = (x64)>>((uintptr_t)43ULL);
  x66 = (x64)&((uintptr_t)8796093022207ULL);
  x67 = (x65)+(x59);
  x68 = x63;
  x69 = x66;
  x70 = x67;
  /*skip*/
  *(uintptr_t*)((out0)+((uintptr_t)0ULL)) = x68;
  *(uintptr_t*)((out0)+((uintptr_t)8ULL)) = x69;
  *(uintptr_t*)((out0)+((uintptr_t)16ULL)) = x70;
  /*skip*/
  return;
}


/*
 * Input Bounds:
 *   in0: [[0x0 ~> 0x34cccccccccb], [0x0 ~> 0x1a6666666664], [0x0 ~> 0x1a6666666664]]
 * Output Bounds:
 *   out0: [[0x0 ~> 0x119999999999], [0x0 ~> 0x8cccccccccc], [0x0 ~> 0x8cccccccccc]]
 */
void fiat_poly1305_carry_square(uintptr_t in0, uintptr_t out0) {
  uintptr_t x2, x3, x4, x1, x5, x6, x0, x9, x17, x20, x18, x21, x10, x22, x19, x11, x13, x26, x14, x27, x12, x7, x15, x30, x16, x31, x8, x29, x23, x34, x32, x35, x33, x25, x36, x39, x28, x40, x38, x41, x24, x43, x44, x45, x37, x47, x48, x42, x46, x49, x50, x51, x52, x53;
  x0 = *(uintptr_t*)((in0)+((uintptr_t)0ULL));
  x1 = *(uintptr_t*)((in0)+((uintptr_t)8ULL));
  x2 = *(uintptr_t*)((in0)+((uintptr_t)16ULL));
  /*skip*/
  /*skip*/
  x3 = (x2)*((uintptr_t)5ULL);
  x4 = (x3)*((uintptr_t)2ULL);
  x5 = (x2)*((uintptr_t)2ULL);
  x6 = (x1)*((uintptr_t)2ULL);
  x7 = (x2)*(x3);
  x8 = sizeof(intptr_t) == 4 ? ((uint64_t)(x2)*(x3))>>32 : ((__uint128_t)(x2)*(x3))>>64 /* TODO this has not been tested */;
  x9 = (x1)*((x4)*((uintptr_t)2ULL));
  x10 = sizeof(intptr_t) == 4 ? ((uint64_t)(x1)*((x4)*((uintptr_t)2ULL)))>>32 : ((__uint128_t)(x1)*((x4)*((uintptr_t)2ULL)))>>64 /* TODO this has not been tested */;
  x11 = (x1)*((x1)*((uintptr_t)2ULL));
  x12 = sizeof(intptr_t) == 4 ? ((uint64_t)(x1)*((x1)*((uintptr_t)2ULL)))>>32 : ((__uint128_t)(x1)*((x1)*((uintptr_t)2ULL)))>>64 /* TODO this has not been tested */;
  x13 = (x0)*(x5);
  x14 = sizeof(intptr_t) == 4 ? ((uint64_t)(x0)*(x5))>>32 : ((__uint128_t)(x0)*(x5))>>64 /* TODO this has not been tested */;
  x15 = (x0)*(x6);
  x16 = sizeof(intptr_t) == 4 ? ((uint64_t)(x0)*(x6))>>32 : ((__uint128_t)(x0)*(x6))>>64 /* TODO this has not been tested */;
  x17 = (x0)*(x0);
  x18 = sizeof(intptr_t) == 4 ? ((uint64_t)(x0)*(x0))>>32 : ((__uint128_t)(x0)*(x0))>>64 /* TODO this has not been tested */;
  x19 = (x17)+(x9);
  x20 = (x19)<(x17);
  x21 = (x20)+(x18);
  x22 = (x21)+(x10);
  x23 = ((x19)>>((uintptr_t)44ULL))|((x22)<<((uintptr_t)20ULL));
  x24 = (x19)&((uintptr_t)17592186044415ULL);
  x25 = (x13)+(x11);
  x26 = (x25)<(x13);
  x27 = (x26)+(x14);
  x28 = (x27)+(x12);
  x29 = (x15)+(x7);
  x30 = (x29)<(x15);
  x31 = (x30)+(x16);
  x32 = (x31)+(x8);
  x33 = (x23)+(x29);
  x34 = (x33)<(x23);
  x35 = (x34)+(x32);
  x36 = ((x33)>>((uintptr_t)43ULL))|((x35)<<((uintptr_t)21ULL));
  x37 = (x33)&((uintptr_t)8796093022207ULL);
  x38 = (x36)+(x25);
  x39 = (x38)<(x36);
  x40 = (x39)+(x28);
  x41 = ((x38)>>((uintptr_t)43ULL))|((x40)<<((uintptr_t)21ULL));
  x42 = (x38)&((uintptr_t)8796093022207ULL);
  x43 = (x41)*((uintptr_t)5ULL);
  x44 = (x24)+(x43);
  x45 = (x44)>>((uintptr_t)44ULL);
  x46 = (x44)&((uintptr_t)17592186044415ULL);
  x47 = (x45)+(x37);
  x48 = (x47)>>((uintptr_t)43ULL);
  x49 = (x47)&((uintptr_t)8796093022207ULL);
  x50 = (x48)+(x42);
  x51 = x46;
  x52 = x49;
  x53 = x50;
  /*skip*/
  *(uintptr_t*)((out0)+((uintptr_t)0ULL)) = x51;
  *(uintptr_t*)((out0)+((uintptr_t)8ULL)) = x52;
  *(uintptr_t*)((out0)+((uintptr_t)16ULL)) = x53;
  /*skip*/
  return;
}


/*
 * Input Bounds:
 *   in0: [[0x0 ~> 0x34cccccccccb], [0x0 ~> 0x1a6666666664], [0x0 ~> 0x1a6666666664]]
 * Output Bounds:
 *   out0: [[0x0 ~> 0x119999999999], [0x0 ~> 0x8cccccccccc], [0x0 ~> 0x8cccccccccc]]
 */
void fiat_poly1305_carry(uintptr_t in0, uintptr_t out0) {
  uintptr_t x0, x1, x2, x3, x4, x6, x7, x5, x8, x9, x10, x11, x12, x13;
  x0 = *(uintptr_t*)((in0)+((uintptr_t)0ULL));
  x1 = *(uintptr_t*)((in0)+((uintptr_t)8ULL));
  x2 = *(uintptr_t*)((in0)+((uintptr_t)16ULL));
  /*skip*/
  /*skip*/
  x3 = x0;
  x4 = ((x3)>>((uintptr_t)44ULL))+(x1);
  x5 = ((x4)>>((uintptr_t)43ULL))+(x2);
  x6 = ((x3)&((uintptr_t)17592186044415ULL))+(((x5)>>((uintptr_t)43ULL))*((uintptr_t)5ULL));
  x7 = ((x6)>>((uintptr_t)44ULL))+((x4)&((uintptr_t)8796093022207ULL));
  x8 = (x6)&((uintptr_t)17592186044415ULL);
  x9 = (x7)&((uintptr_t)8796093022207ULL);
  x10 = ((x7)>>((uintptr_t)43ULL))+((x5)&((uintptr_t)8796093022207ULL));
  x11 = x8;
  x12 = x9;
  x13 = x10;
  /*skip*/
  *(uintptr_t*)((out0)+((uintptr_t)0ULL)) = x11;
  *(uintptr_t*)((out0)+((uintptr_t)8ULL)) = x12;
  *(uintptr_t*)((out0)+((uintptr_t)16ULL)) = x13;
  /*skip*/
  return;
}


/*
 * Input Bounds:
 *   in0: [[0x0 ~> 0x119999999999], [0x0 ~> 0x8cccccccccc], [0x0 ~> 0x8cccccccccc]]
 *   in1: [[0x0 ~> 0x119999999999], [0x0 ~> 0x8cccccccccc], [0x0 ~> 0x8cccccccccc]]
 * Output Bounds:
 *   out0: [[0x0 ~> 0x34cccccccccb], [0x0 ~> 0x1a6666666664], [0x0 ~> 0x1a6666666664]]
 */
void fiat_poly1305_add(uintptr_t in0, uintptr_t in1, uintptr_t out0) {
  uintptr_t x0, x3, x1, x4, x2, x5, x6, x7, x8, x9, x10, x11;
  x0 = *(uintptr_t*)((in0)+((uintptr_t)0ULL));
  x1 = *(uintptr_t*)((in0)+((uintptr_t)8ULL));
  x2 = *(uintptr_t*)((in0)+((uintptr_t)16ULL));
  /*skip*/
  x3 = *(uintptr_t*)((in1)+((uintptr_t)0ULL));
  x4 = *(uintptr_t*)((in1)+((uintptr_t)8ULL));
  x5 = *(uintptr_t*)((in1)+((uintptr_t)16ULL));
  /*skip*/
  /*skip*/
  x6 = (x0)+(x3);
  x7 = (x1)+(x4);
  x8 = (x2)+(x5);
  x9 = x6;
  x10 = x7;
  x11 = x8;
  /*skip*/
  *(uintptr_t*)((out0)+((uintptr_t)0ULL)) = x9;
  *(uintptr_t*)((out0)+((uintptr_t)8ULL)) = x10;
  *(uintptr_t*)((out0)+((uintptr_t)16ULL)) = x11;
  /*skip*/
  return;
}


/*
 * Input Bounds:
 *   in0: [[0x0 ~> 0x119999999999], [0x0 ~> 0x8cccccccccc], [0x0 ~> 0x8cccccccccc]]
 *   in1: [[0x0 ~> 0x119999999999], [0x0 ~> 0x8cccccccccc], [0x0 ~> 0x8cccccccccc]]
 * Output Bounds:
 *   out0: [[0x0 ~> 0x34cccccccccb], [0x0 ~> 0x1a6666666664], [0x0 ~> 0x1a6666666664]]
 */
void fiat_poly1305_sub(uintptr_t in0, uintptr_t in1, uintptr_t out0) {
  uintptr_t x0, x3, x1, x4, x2, x5, x6, x7, x8, x9, x10, x11;
  x0 = *(uintptr_t*)((in0)+((uintptr_t)0ULL));
  x1 = *(uintptr_t*)((in0)+((uintptr_t)8ULL));
  x2 = *(uintptr_t*)((in0)+((uintptr_t)16ULL));
  /*skip*/
  x3 = *(uintptr_t*)((in1)+((uintptr_t)0ULL));
  x4 = *(uintptr_t*)((in1)+((uintptr_t)8ULL));
  x5 = *(uintptr_t*)((in1)+((uintptr_t)16ULL));
  /*skip*/
  /*skip*/
  x6 = (((uintptr_t)35184372088822ULL)+(x0))-(x3);
  x7 = (((uintptr_t)17592186044414ULL)+(x1))-(x4);
  x8 = (((uintptr_t)17592186044414ULL)+(x2))-(x5);
  x9 = x6;
  x10 = x7;
  x11 = x8;
  /*skip*/
  *(uintptr_t*)((out0)+((uintptr_t)0ULL)) = x9;
  *(uintptr_t*)((out0)+((uintptr_t)8ULL)) = x10;
  *(uintptr_t*)((out0)+((uintptr_t)16ULL)) = x11;
  /*skip*/
  return;
}


/*
 * Input Bounds:
 *   in0: [[0x0 ~> 0x119999999999], [0x0 ~> 0x8cccccccccc], [0x0 ~> 0x8cccccccccc]]
 * Output Bounds:
 *   out0: [[0x0 ~> 0x34cccccccccb], [0x0 ~> 0x1a6666666664], [0x0 ~> 0x1a6666666664]]
 */
void fiat_poly1305_opp(uintptr_t in0, uintptr_t out0) {
  uintptr_t x0, x1, x2, x3, x4, x5, x6, x7, x8;
  x0 = *(uintptr_t*)((in0)+((uintptr_t)0ULL));
  x1 = *(uintptr_t*)((in0)+((uintptr_t)8ULL));
  x2 = *(uintptr_t*)((in0)+((uintptr_t)16ULL));
  /*skip*/
  /*skip*/
  x3 = ((uintptr_t)35184372088822ULL)-(x0);
  x4 = ((uintptr_t)17592186044414ULL)-(x1);
  x5 = ((uintptr_t)17592186044414ULL)-(x2);
  x6 = x3;
  x7 = x4;
  x8 = x5;
  /*skip*/
  *(uintptr_t*)((out0)+((uintptr_t)0ULL)) = x6;
  *(uintptr_t*)((out0)+((uintptr_t)8ULL)) = x7;
  *(uintptr_t*)((out0)+((uintptr_t)16ULL)) = x8;
  /*skip*/
  return;
}


/*
 * Input Bounds:
 *   in0: [[0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0xff], [0x0 ~> 0x3]]
 * Output Bounds:
 *   out0: [[0x0 ~> 0x119999999999], [0x0 ~> 0x8cccccccccc], [0x0 ~> 0x8cccccccccc]]
 */
void fiat_poly1305_from_bytes(uintptr_t in0, uintptr_t out0) {
  uintptr_t x16, x15, x14, x13, x12, x11, x10, x9, x8, x7, x6, x5, x4, x3, x2, x1, x0, x33, x32, x31, x30, x29, x28, x34, x22, x21, x20, x19, x18, x17, x27, x26, x25, x24, x23, x35, x38, x39, x40, x37, x36, x41, x42, x43, x44, x45;
  x0 = *(uintptr_t*)((in0)+((uintptr_t)0ULL));
  x1 = *(uintptr_t*)((in0)+((uintptr_t)8ULL));
  x2 = *(uintptr_t*)((in0)+((uintptr_t)16ULL));
  x3 = *(uintptr_t*)((in0)+((uintptr_t)24ULL));
  x4 = *(uintptr_t*)((in0)+((uintptr_t)32ULL));
  x5 = *(uintptr_t*)((in0)+((uintptr_t)40ULL));
  x6 = *(uintptr_t*)((in0)+((uintptr_t)48ULL));
  x7 = *(uintptr_t*)((in0)+((uintptr_t)56ULL));
  x8 = *(uintptr_t*)((in0)+((uintptr_t)64ULL));
  x9 = *(uintptr_t*)((in0)+((uintptr_t)72ULL));
  x10 = *(uintptr_t*)((in0)+((uintptr_t)80ULL));
  x11 = *(uintptr_t*)((in0)+((uintptr_t)88ULL));
  x12 = *(uintptr_t*)((in0)+((uintptr_t)96ULL));
  x13 = *(uintptr_t*)((in0)+((uintptr_t)104ULL));
  x14 = *(uintptr_t*)((in0)+((uintptr_t)112ULL));
  x15 = *(uintptr_t*)((in0)+((uintptr_t)120ULL));
  x16 = *(uintptr_t*)((in0)+((uintptr_t)128ULL));
  /*skip*/
  /*skip*/
  x17 = (x16)<<((uintptr_t)41ULL);
  x18 = (x15)<<((uintptr_t)33ULL);
  x19 = (x14)<<((uintptr_t)25ULL);
  x20 = (x13)<<((uintptr_t)17ULL);
  x21 = (x12)<<((uintptr_t)9ULL);
  x22 = (x11)*((uintptr_t)2ULL);
  x23 = (x10)<<((uintptr_t)36ULL);
  x24 = (x9)<<((uintptr_t)28ULL);
  x25 = (x8)<<((uintptr_t)20ULL);
  x26 = (x7)<<((uintptr_t)12ULL);
  x27 = (x6)<<((uintptr_t)4ULL);
  x28 = (x5)<<((uintptr_t)40ULL);
  x29 = (x4)<<((uintptr_t)32ULL);
  x30 = (x3)<<((uintptr_t)24ULL);
  x31 = (x2)<<((uintptr_t)16ULL);
  x32 = (x1)<<((uintptr_t)8ULL);
  x33 = x0;
  x34 = (x33)+((x32)+((x31)+((x30)+((x29)+(x28)))));
  x35 = (x34)>>((uintptr_t)44ULL);
  x36 = (x34)&((uintptr_t)17592186044415ULL);
  x37 = (x22)+((x21)+((x20)+((x19)+((x18)+(x17)))));
  x38 = (x27)+((x26)+((x25)+((x24)+(x23))));
  x39 = (x35)+(x38);
  x40 = (x39)>>((uintptr_t)43ULL);
  x41 = (x39)&((uintptr_t)8796093022207ULL);
  x42 = (x40)+(x37);
  x43 = x36;
  x44 = x41;
  x45 = x42;
  /*skip*/
  *(uintptr_t*)((out0)+((uintptr_t)0ULL)) = x43;
  *(uintptr_t*)((out0)+((uintptr_t)8ULL)) = x44;
  *(uintptr_t*)((out0)+((uintptr_t)16ULL)) = x45;
  /*skip*/
  return;
}


