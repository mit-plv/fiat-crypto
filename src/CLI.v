...
Module ForExtraction.
  ...
  (** ** Shell quoting for the "Autogenerated: ..." header line

      The header of every generated file records the command line
      used to produce it, so that it can be copied into a shell and
      re-run.  This quoter is therefore a security boundary: in the
      web build (js_of_ocaml), [argv] comes from a [?argv=] URL that
      anybody can craft, and the victim may copy the resulting
      "Autogenerated:" line into a shell.

      We use an allow-list, following Python's [shlex.quote]: a word is
      emitted bare only if it is non-empty and consists solely of
      characters that are safe in every POSIX-compatible shell (ASCII
      letters, digits, and [_-.,:@%+]).  Everything else is wrapped in
      single quotes, with each embedded single quote rendered as
      ['"'"'].  The previous implementation used a deny-list that
      omitted newline, tab, carriage return and all other control
      characters, so an argument such as "p256<newline>touch pwned"
      was emitted bare and the newline ran [touch pwned] as a second
      command when the line was pasted (scrutineer finding #2511).

      Like the previous implementation (and unlike [shlex.quote]) we
      also quote "/", so that we don't change quoting behavior based
      on Windows vs Linux paths, and "=".  The output for every
      argument that the old quoter handled correctly is unchanged.

      Arguments containing control characters (bytes below 0x20, and
      0x7f) get special treatment, because a raw control character in
      the header line is itself a problem: several backends (Go, Zig)
      wrap each header line in a [//] line comment, so a raw newline
      would end the comment and turn the remainder of the argument
      into source code.  Such arguments are emitted using ANSI-C
      quoting, [$'...'], with every control character, backslash and
      single quote escaped (as [\n], [\t], [\r], [\\] or a three-digit
      octal escape), so the emitted line never contains a raw control
      character.  [$'...'] is understood by bash, zsh, ksh and by any
      shell implementing POSIX.1-2024.  In shells that do not
      understand it (e.g. dash), the word is still an ordinary
      single-quoted string preceded by a literal [$]: it produces the
      wrong argument, but cannot execute anything, because we never
      emit a single quote inside the [$'...'] form (we use the octal
      escape [\047] instead of [\']).  Legitimate arguments never
      contain control characters, so this form does not appear in any
      checked-in generated file. *)
  Definition shell_safe_char (ch : ascii) : bool
    := let n := N_of_ascii ch in
       (((65 <=? n) && (n <=? 90)) (* A-Z *)
        || ((97 <=? n) && (n <=? 122)) (* a-z *)
        || ((48 <=? n) && (n <=? 57)))%N (* 0-9 *)
       || List.existsb (fun c => c =? ch)%char ["_"; "-"; "."; ","; ":"; "@"; "%"; "+"]%char.

  Definition is_control_char (ch : ascii) : bool
    := let n := N_of_ascii ch in
       ((n <? 32) || (n =? 127))%N.

  (** Three-digit octal escape, e.g. [\047] for the single quote *)
  Definition octal_escape (ch : ascii) : string
    := let n := N_of_ascii ch in
       let digit d := ascii_of_N (48 + ((n / d) mod 8))%N in
       String "\" (String (digit 64%N) (String (digit 8%N) (String (digit 1%N) ""))).

  Definition ansi_c_escape_char (ch : ascii) : string
    := if (ch =? "\")%char then "\\"
       else if (ch =? "'")%char then octal_escape ch
       else if (ch =? "010")%char then "\n" (* newline *)
       else if (ch =? "009")%char then "\t" (* tab *)
       else if (ch =? "013")%char then "\r" (* carriage return *)
       else if is_control_char ch then octal_escape ch
       else String ch "".

  Definition quote (s : string) : string
    := let chars := String.list_ascii_of_string s in
       if List.forallb shell_safe_char chars && negb (String.length s =? 0)%nat
       then s
       else if List.existsb is_control_char chars
       then "$'" ++ String.concat "" (List.map ansi_c_escape_char chars) ++ "'"
