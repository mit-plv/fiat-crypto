...
Module ForExtraction.
  ...
  (** Takes a list of strings to be joined by newlines and then commented.  Internal newlines are left alone. *)
  (* TODO: Move this somewhere else, probably *)
  Definition comment_block (s : list string) : list string :=
  ...
  Class PipelineAPI :=
    {
      ...
      (* TODO: make handling of commenting more uniform *)
      Synthesize : forall (args : ArgsT) (function_name_prefix : string),
          list string (* extra comment header (already wrapped in a comment block) *)
          * list (string * Pipeline.ErrorT (list string)) (* result of synthesis *)
          * MSetPositive.PositiveSet.t (* types used *);

      (** The check_args function makes sure the arguments satisfy the preconditions *)
      check_args : forall (args : ArgsT) {T} (res : Pipeline.ErrorT T),
          Pipeline.ErrorT T
    }.
      ...
      Definition PipelineLines
        ...
        := let prefix := ("fiat_" ++ curve_description ++ "_")%string in
           let '(extra_comment_header, res, types_used)
               := Synthesize args prefix in
           let header :=
               (comment_block
                  ((["Autogenerated";
                       "curve description: " ++ curve_description]%string)
                     ++ show_lines_args args)
                  ++ [""]
           ...
           inl
             ([("check_args" ++ NewLine ++ String.concat NewLine header,
                check_args args (ErrorT.Success header))%string]
                ++ res)%list.
  ...
  Module UnsaturatedSolinas.
    Local Instance api : PipelineAPI
      := {
          ...
          Synthesize
          := fun '(_, (n, machine_wordsize, s, c, requests)) prefix
             => UnsaturatedSolinas.Synthesize n s c machine_wordsize prefix requests;

          check_args
          := fun '(_, (n, machine_wordsize, s, c, requests)) T res
             => UnsaturatedSolinas.check_args n s c machine_wordsize res
        }.
  ...
  Module WordByWordMontgomery.
    Local Instance api : PipelineAPI
      := {
          ...
          Synthesize
          := fun '(_, (machine_wordsize, m, requests)) prefix
             => WordByWordMontgomery.Synthesize m machine_wordsize prefix requests;

          check_args
          := fun '(_, (machine_wordsize, m, requests)) T res
             => WordByWordMontgomery.check_args m machine_wordsize res
        }.
  ...
  Module SaturatedSolinas.
    Local Instance api : PipelineAPI
      := {
          ...
          Synthesize
          := fun '(_, (machine_wordsize, s, c, requests)) prefix
             => SaturatedSolinas.Synthesize s c machine_wordsize prefix requests;

          check_args
          := fun '(_, (machine_wordsize, s, c, requests)) T res
             => SaturatedSolinas.check_args s c machine_wordsize res
        }.
