...
(*
  ...
  There are 9 limbs, each of length 57.
...
Definition input_bounds := Some ((repeat (Some r[0 ~> 2 * m * (2^57 - 1)]%zrange) 8) ++ [Some r[0 ~> 2 * m * (2^56 - 1)]%zrange]).
Definition output_bounds := Some ((repeat (Some r[0 ~> 2 * M * (2^57 - 1)]%zrange) 8) ++ [Some r[0 ~> 2 * M * (2^56 - 1)]%zrange]).
