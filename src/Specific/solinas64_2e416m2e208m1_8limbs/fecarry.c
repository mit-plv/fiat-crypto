static void fecarry(uint64_t out[8], const uint64_t in1[8]) {
  { const uint64_t x13 = in1[7];
  { const uint64_t x14 = in1[6];
  { const uint64_t x12 = in1[5];
  { const uint64_t x10 = in1[4];
  { const uint64_t x8 = in1[3];
  { const uint64_t x6 = in1[2];
  { const uint64_t x4 = in1[1];
  { const uint64_t x2 = in1[0];
  { uint64_t x15 = (x8 >> 0x34);
  { uint64_t x16 = (x8 & 0xfffffffffffff);
  { uint64_t x17 = (x13 >> 0x34);
  { uint64_t x18 = (x13 & 0xfffffffffffff);
  { uint64_t x19 = ((0x10000000000000 * x17) + x18);
  { uint64_t x20 = (x19 >> 0x34);
  { uint64_t x21 = (x19 & 0xfffffffffffff);
  { uint64_t x22 = ((x15 + x10) + x20);
  { uint64_t x23 = (x22 >> 0x34);
  { uint64_t x24 = (x22 & 0xfffffffffffff);
  { uint64_t x25 = (x2 + x20);
  { uint64_t x26 = (x25 >> 0x34);
  { uint64_t x27 = (x25 & 0xfffffffffffff);
  { uint64_t x28 = (x23 + x12);
  { uint64_t x29 = (x28 >> 0x34);
  { uint64_t x30 = (x28 & 0xfffffffffffff);
  { uint64_t x31 = (x26 + x4);
  { uint64_t x32 = (x31 >> 0x34);
  { uint64_t x33 = (x31 & 0xfffffffffffff);
  { uint64_t x34 = (x29 + x14);
  { uint64_t x35 = (x34 >> 0x34);
  { uint64_t x36 = (x34 & 0xfffffffffffff);
  { uint64_t x37 = (x32 + x6);
  { uint64_t x38 = (x37 >> 0x34);
  { uint64_t x39 = (x37 & 0xfffffffffffff);
  { uint64_t x40 = (x35 + x21);
  { uint64_t x41 = (x40 >> 0x34);
  { uint64_t x42 = (x40 & 0xfffffffffffff);
  { uint64_t x43 = (x38 + x16);
  { uint64_t x44 = (x43 >> 0x34);
  { uint64_t x45 = (x43 & 0xfffffffffffff);
  { uint64_t x46 = ((0x10000000000000 * x41) + x42);
  { uint64_t x47 = (x46 >> 0x34);
  { uint64_t x48 = (x46 & 0xfffffffffffff);
  { uint64_t x49 = ((x44 + x24) + x47);
  { uint64_t x50 = (x49 >> 0x34);
  { uint64_t x51 = (x49 & 0xfffffffffffff);
  { uint64_t x52 = (x27 + x47);
  { uint64_t x53 = (x52 >> 0x34);
  { uint64_t x54 = (x52 & 0xfffffffffffff);
  out[0] = x54;
  out[1] = (x53 + x33);
  out[2] = x39;
  out[3] = x45;
  out[4] = x51;
  out[5] = (x50 + x30);
  out[6] = x36;
  out[7] = x48;
  }}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}
}
