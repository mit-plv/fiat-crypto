...
Module debugging_solinas_reduction.
  ...
  Section __.
    ...
    Definition square_no_reduce base (p : list Z) :=
      ...
      let square := Saturated.Rows.flatten weight 8 square_rows in
      fst square.
    ...
    Time Compute
         ...
         (Pipeline.BoundsPipelineToString
            ...
            ltac:(let n := (eval cbv in n) in
                  let r := Reify (squaremod base s c n) in
                  exact r)
                   (fun _ _ => [])
 ...
 */
void mul(uint64_t out1[4], const uint64_t arg1[4]) {
  uint64_t x1;
  uint64_t x2;
  uint64_t x3;
  uint64_t x4;
  ...
  fiatuint128 x100;
  uint64_t x101;
  fiatmulx_u64(&x1, &x2, (arg1[0]), (arg1[3]));
  fiatmulx_u64(&x3, &x4, (arg1[0]), (arg1[2]));
  fiatmulx_u64(&x5, &x6, (arg1[0]), (arg1[1]));
  fiatmulx_u64(&x7, &x8, (arg1[3]), (arg1[2]));
  fiatmulx_u64(&x9, &x10, (arg1[3]), (arg1[1]));
  fiataddcarryx_u64(&x11, &x12, 0x0, x6, x3);
  fiataddcarryx_u64(&x13, &x14, x12, x4, x1);
  fiataddcarryx_u64(&x15, &x16, x14, x2, x9);
  fiataddcarryx_u64(&x17, &x18, x16, x10, x7);
  x19 = (x18 + x8);
  fiatmulx_u64(&x20, &x21, (arg1[1]), (arg1[2]));
  fiataddcarryx_u64(&x22, &x23, 0x0, x13, x20);
  fiataddcarryx_u64(&x24, &x25, x23, x15, x21);
  fiataddcarryx_u64(&x26, &x27, x25, x17, 0x0);
  fiataddcarryx_u64(&x28, &x29, x27, x19, 0x0);
  fiataddcarryx_u64(&x30, &x31, 0x0, x5, x5);
  fiataddcarryx_u64(&x32, &x33, x31, x11, x11);
  fiataddcarryx_u64(&x34, &x35, x33, x22, x22);
  fiataddcarryx_u64(&x36, &x37, x35, x24, x24);
  fiataddcarryx_u64(&x38, &x39, x37, x26, x26);
  fiataddcarryx_u64(&x40, &x41, x39, x28, x28);
  x42 = (((uint64_t)x41 + x29) + (uint64_t)x29);
  fiatmulx_u64(&x43, &x44, (arg1[3]), (arg1[3]));
  fiatmulx_u64(&x45, &x46, (arg1[2]), (arg1[2]));
  fiatmulx_u64(&x47, &x48, (arg1[1]), (arg1[1]));
  fiatmulx_u64(&x49, &x50, (arg1[0]), (arg1[0]));
  fiataddcarryx_u64(&x51, &x52, 0x0, x50, x30);
  fiataddcarryx_u64(&x53, &x54, x52, x47, x32);
  fiataddcarryx_u64(&x55, &x56, x54, x48, x34);
  fiataddcarryx_u64(&x57, &x58, x56, x45, x36);
  fiataddcarryx_u64(&x59, &x60, x58, x46, x38);
  fiataddcarryx_u64(&x61, &x62, x60, x43, x40);
  fiataddcarryx_u64(&x63, &x64, x62, x44, x42);
  fiatmulx_u64(&x65, &x66, UINT8_C(0x26), x63);
  fiatmulx_u64(&x67, &x68, UINT8_C(0x26), x61);
  fiatmulx_u64(&x69, &x70, UINT8_C(0x26), x59);
  fiatmulx_u64(&x71, &x72, UINT8_C(0x26), x57);
  fiataddcarryx_u64(&x73, &x74, 0x0, x51, x69);
  fiataddcarryx_u64(&x75, &x76, x74, x53, x67);
  fiataddcarryx_u64(&x77, &x78, x76, x55, x65);
  x79 = (x78 + x66);
  fiataddcarryx_u64(&x80, &x81, 0x0, x49, x71);
  fiataddcarryx_u64(&x82, &x83, x81, x73, x72);
  fiataddcarryx_u64(&x84, &x85, x83, x75, x70);
  fiataddcarryx_u64(&x86, &x87, x85, x77, x68);
  x88 = (x87 + x79);
  fiatmulx_u64(&x89, &x90, UINT8_C(0x26), x88);
  fiataddcarryx_u64(&x91, &x92, 0x0, x80, x89);
  fiataddcarryx_u64(&x93, &x94, x92, x82, 0x0);
  fiataddcarryx_u64(&x95, &x96, x94, x84, 0x0);
  fiataddcarryx_u64(&x97, &x98, x96, x86, 0x0);
  fiatcmovznz_u64(&x99, x98, 0x0, UINT8_C(0x26));
  x100 = ((fiatuint128)x99 + x91);
  x101 = (uint64_t)(x100 & UINT64_C(0xffffffffffffffff));
  out1[0] = x101;
  out1[1] = x93;
  out1[2] = x95;
  out1[3] = x97;
}"", {| bitwidths_used := [uint128, uint1, uint64] ; addcarryx_lg_splits := [64] ; mulx_lg_splits := [64] ; cmovznz_bitwidths := [uint64] ; value_barrier_bitwidths := [] ; typedefs_used := [] |})"
     : string
Finished transaction in 4.494 secs (4.41u,0.066s) (successful)
*)
