(** * Push-Button Synthesis of Bitcoin Multiplication *)
...
Require Import Crypto.PushButtonSynthesis.BitcoinMultiplicationReificationCache.
Require Import Crypto.BitcoinMultiplication.GeneralizedMultiplicationAlgorithm.
...
Import COperationSpecifications.BitcoinMultiplication.
...
Check (fun summary correctness => (fun fname arg_var_data out_var_data =>
   match
     CorrectnessStringification.dyn_context.cons (weight limb_size 1) "weight"
       (CorrectnessStringification.dyn_context.cons (eval (weight limb_size 1) n)
          "eval" CorrectnessStringification.dyn_context.nil), 
     summary, correctness
   with
   | ctx', summary', correctness' => _
   end)).
