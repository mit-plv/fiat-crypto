(** * Direct reification of PHOAS Representation of Gallina *)
(** The reification procedure introduces extra [let]s and [match]es,
    so we don't get something syntactically equal.  Without primitive
    projections, we don't get something judgmentally equal. *)
...
Ltac reifyf base_type_code interp_base_type op var e :=
  ...
  let mkVar T ex := constr:(SmartVar (base_type_code:=base_type_code) (interp_base_type:=interp_base_type) (op:=op) (var:=var) (t:=T) ex) in
  ...
  | (fun x : ?T => ?C) =>
    ...
    lazymatch constr:(fun (x : T) (not_x : interp_flat_type_gen var t) (_ : reify_var_for_in_is base_type_code x t not_x) =>
  ...
  | match ?ev with pair a b => @?eC a b end =>
    ...
    let C := lazymatch C
    with (fun (a : ?A) (b : ?B) => ?C) => constr:(fun ab : prod A B => match fst ab, snd ab with a, b => C end) end in
    mkLet v C
  | ?x =>
...
Ltac reify_abs base_type_code interp_base_type op var e :=
  ...
  | (fun x : ?T => ?C) =>
    ...
    let maybe_x := fresh x in
    let not_x := fresh x in
    lazymatch constr:(fun (x : T) (not_x : var t) (_ : reify_var_for_in_is base_type_code x (Tbase t) not_x) =>
...
Ltac Reify base_type_code interp_base_type op e :=
  lazymatch constr:(fun (var : base_type_code -> Type) => (_ : reify_abs (@exprf base_type_code interp_base_type op var) e)) with
    (fun var => ?C) => constr:(fun (var : base_type_code -> Type) => C) (* copy the term but not the type cast *)
  end.
