...
Lemma lctz_word_slu (w : word) : (w > 0) -> lctz 64 (Zmod.slu w 1) = 1 + lctz 64 w.
Proof.
    intros H.
    ...
    { pose proof (bits.unsigned_range w width_nonneg).
        ...
        destruct E.
        replace (Zmod.unsigned w) with (2^63) by lia.
        reflexivity. }
...
Proof.
    destruct (Zmod.eqb w (bits.of_Z _ 0)) eqn: Heq.
    {
        eapply Zmod.eqb_eq in Heq. rewrite Heq in *; 
        simpl; lia.
    }
    {
...
#[export] Instance spec_of_br_ctz : spec_of "br_ctz" := 
    ...
    {
        requires t m := True;
        ensures T M := T = t /\ M = m /\ Zmod.unsigned count = (lctz 64 value)
    }.
...
Proof.
    ...
    (fun v t m value_ count tmp => PrimitivePair.pair.mk (* precondition *)
      (
        v = Zmod.unsigned count /\ Zmod.unsigned tmp = (Zmod.unsigned value * 2^count) mod 2^64 /\ 
      lctz 64 (Zmod.unsigned tmp) = lctz 64 (Zmod.unsigned value) + (Zmod.unsigned count) /\ value = value_)
    (fun T M VALUE COUNT TMP => (* postcondition *)
      T = t /\ M = m /\ 64 = lctz 64 (Zmod.unsigned value) + Zmod.unsigned COUNT))
    ...
    { 
        ...
        rewrite Zmod.unsigned_0, Z.pow_0_r. ZnWords.
    }
    { 
        ...
        all: assert (Htmp : lctz 64 tmp = lctz 64 x1 + 1) by (cbv [tmp]; rewrite shamt_of_Z_small by lia; rewrite lctz_word_slu; ZnWords).
