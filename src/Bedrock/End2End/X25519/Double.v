...
Definition double := func! (ox, oy, oz, ot, X, Y, Z, T) {
  ...
  stackalloc 40 as cZ;
  fe25519_sub(cZ, trZ, trX); (* Also gonna hit bounds issue *)
  stackalloc 40 as cX;
...
Section WithParameters.
  ...
  Context {a d: F M_pos} (* Figure this out... later *)
          {nonzero_a : a <> F.zero}
          {square_a : exists sqrt_a, (F.mul sqrt_a sqrt_a) = a}
          {nonsquare_d : forall x, (F.mul x x) <> d}.
  Context {a_eq_minus1:a = F.opp F.one} {twice_d} {k_eq_2d:twice_d = (F.add d d)}. (* Really don't know why this is needed for m1double *)
...
Proof.
  ...
  admit. (* bounds issue on cZ -- 2 lines up needs carry_sub instead *)
