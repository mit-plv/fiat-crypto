...
Module ForExtraction.
  ...
  Definition parse_use_saturated_bounds (s : string) : option bool
    := parse_bool s.
  ...
  Definition use_saturated_bounds_help
    := "  use_saturated_bounds    Whether to use saturated bounds of 2^bitwidth-1 ('true') or not ('false')".
  ...
  Module UnsaturatedSolinas.
    ...
    : list (string * Pipeline.ErrorT (list string)) + list string
      := let prefix := ("fiat_" ++ curve_description ++ "_")%string in
         ...
         let str_c := c in
         let str_s := s in
         let str_use_saturated_bounds := use_saturated_bounds in
         ...
         match parse_s s, parse_c c, parse_use_saturated_bounds use_saturated_bounds with
         | Some s, Some c, Some use_saturated_bounds
           => let '(res, types_used)
                 := UnsaturatedSolinas.Synthesize n s c machine_wordsize use_saturated_bounds prefix requests in
             let header :=
                 ((["/* Autogenerated */";
                       ...
                       "/* use_saturated_bounds = " ++ show false use_saturated_bounds ++ " (from """ ++ str_use_saturated_bounds ++ """) */";
                       ""]%string)
                     ...
                     ++ (String.concat
                           ...
                           (OptionList.Option.List.map
                              ...
                              [(("s", s), if sv then true else false);
                                 ...
                                 (("use_saturated_bounds", use_saturated_bounds), if use_saturated_boundsv then true else false)]))]
         end.
    ...
    Definition ProcessedLines
      ...
      := match CollectErrors (PipelineLines curve_description n s c machine_wordsize use_saturated_bounds requests) with
         | inl ls
           => inl
    ...
    Definition Pipeline
               ...
               (error : list string -> A)
      : A
      := match ProcessedLines curve_description n s c machine_wordsize use_saturated_bounds requests with
         | inl s => success s
         | inr s => error s
         end.
    ...
    Definition PipelineMain
      ...
      : A
      := match argv with
         | _::curve_description::n::s::c::machine_wordsize::use_saturated_bounds::requests
           => Pipeline
               curve_description n s c machine_wordsize use_saturated_bounds requests
               success
               error
         | nil => error ["empty argv"]
         | prog::args
           => error ["Expected arguments curve_description, n, s, c, machine_wordsize, use_saturated_bounds, [function_to_synthesize*] got " ++ show false (List.length args) ++ " arguments in " ++ prog;
                       "";
