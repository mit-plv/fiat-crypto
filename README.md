...
Language.Pre.v
    ↑
Language.v ←─────────── Identifier.v ←─────────── Language.API.v
    ↑                                                    ↑
    ├─────────────────────┐                       ┌──────┴────────────────────────────────────────┐
MiscCompilerPasses.v   UnderLets.v                │                          PushButtonSynthesis/ReificationCache.v
                          ↑                       │
                          └──────────────┬────────┘
                                         │
               ┌─────────────────────────┴───────────────────────────────────────────┐
      AbstractInterpretation.v                                                   "Rewriter"
               ↑
      Stringification.Language.v
               ↑
               ├───────────────────────────────────────────────────────────┐
              IR.v
               ↑
               ├───────────────────────┐
      Stringification.C.v   Stringification.Rust.v
               ↑                       ↑
               └──────────────┬────────┘
...
AbstractInterpretation.v  "Rewriter"                                               │                               │
         ↑                  ↑ ┌────────────────────────────────────────────────────┘                               │
Stringification.Language.v  │ │               Arithmetic.v                                                         │
↑    ↑  ┌───────────────────┴─┘                   ↑                                                                │
│  BoundsPipeline.v                     COperationSpecifications.v                                                 │
│     ↑                                           ↑                                                                │
│     └─────────────────────────┬─────────────────┴────────────────────────────────────────────────────────────────┘
Stringification.C.v    PushButtonSynthesis.v ←── Toplevel2.v ←───────────┐
↑                               ↑                                        │
...
- Language.Pre.v: A few definitions which are used in writing out
...
- Identifier.v: Defines the specific type of base types and
  identifiers.  Will eventually be autogenerated.

- Language.API.v: Specializes the type of PHOAS expressions to the
...
- Stringification.Language.v: defines a printer (Show instance) for the
...
- IR.v: Defines a common IR for C and Rust (and maybe eventually other
  languages), and builds most of the infrastructure necessary for
  instantiating the LanguageSpecification API for a language with
  pointers and function calls

- Stringification.C.v: conversion to C code as strings.  Instantiates
  the API defined in Stringification.Language.

- Stringification.Rust.v: conversion to Rust code as strings.
  Instantiates the API defined in Stringification.Language.
...
Rewriter.Reify.v ←─────┐   Rewriter.Wf.v   Rewriter.InterpProofs.v              │                  │
                       │              ↖        ↗                                │                  │
Rewriter.Rules.v       └─────── Rewriter.AllTactics.v ──────────────────────────┘                  │
    ↑                                      ↑       ┌───────────────────────────────────────────────┘
Rewriter.RulesProofs.v           Rewriter.AllTacticsExtra.v
    ↑                                      ↑
    ├────────┬─────────────┬───────────────┴────────┬─────────────────────────────┐
...
- Rewriter.Rules.v: Defines the list of types of the rewrite rules that
  will be reified.  Largely independent of the expression language.

- Rewriter.RulesProofs.v: Proves all of the Gallina versions of the
...
- Rewriter.AllTacticsExtra.v: Specializes `Rewriter.AllTactics.v` to
  what's defined in `Identifier.v`

- Rewriter/{NBE, Arith, ArithWithCasts, StripLiteralCasts, ToFancy,
  ToFancyWithCasts}.v: Use the tactic from Rewriter.AllTactics.v
  together with the proven list of rewrite rules from
  Rewriter.RulesProofs.v to reify and reduce the corresponding pass and
  generate a rewriter.

- Rewriter.All.v: `Definition`less file that `Export`s the rewriters
